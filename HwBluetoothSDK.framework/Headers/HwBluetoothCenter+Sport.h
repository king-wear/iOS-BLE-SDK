...
typedef void (^HwActivitiesCallback)(NSArray<HwActivity *> * _Nullable sportArray, NSError *_Nullable error);
...
/*! @brief
 判断是否在睡眠[determin if you are sleeping]
 
 @param callback 是否回调[call back or not]
 @return 任务[task]
 */
- (HwBluetoothTask *_Nullable) getSleepStateWithCallback:(HwBoolCallback _Nullable)callback;
